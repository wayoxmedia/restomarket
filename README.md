...
cd html
```
This will give you a shell inside the container. Make sure you are in the html folder, this is where the FrontEnd (admin) code is located and the package.json file lives here.
...
### GULP
Gulp is also used to compile the SCSS files into CSS files. You can also use Gulp to watch the SCSS files for changes and compile them automatically.
```sh
npm run gulp
```
